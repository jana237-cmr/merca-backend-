...
export class UsersService {
  ...
  // MERCA CERCLE : mon code personnel + statistiques de parrainage
  async myCircle(id: string) {
    const me = await this.users.findOne({ where: { id } });
    const invited = await this.users.find({ where: { referredBy: id } });
    return {
      code: me.referralCode,
      invitedCount: invited.length,
      activeCount: invited.filter(u => u.referralRewarded).length,
    };
  }
}
...
export class UsersController {
  ...
  @Get('circle')
  circle(@Req() req: any) {
    return this.users.myCircle(req.user.userId);
  }
}
